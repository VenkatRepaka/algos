...
public class SuffixTreeFromArray {
    ...
    private int lcpOfSuffixes(String text, int i, int j, int equal) {
        ...
        while (i + lcp < text.length() && j + lcp < text.length()){
            if (text.charAt(i + lcp) == text.charAt(j + lcp)) {
                lcp = lcp + 1;
            } else {
                break;
            }
    ...
    public int[] computeLCPArray(String text, int[] order) {
        int[] lcpArray = new int[text.length()-1];
        int lcp = 0;
        int[] posInOrder = invertSuffixArray(order);
        int suffix = order[0];
        int nextSuffix = 0, orderIndex = 0;
    ...
    public Map<Integer, List<Edge>> SuffixTreeFromSuffixArray(
            int[] suffixArray,
            int[] lcpArray,
            final String text) {
    ...
    static public void main(String[] args) throws IOException {
        SuffixTreeFromArray suffixTreeFromArray = new SuffixTreeFromArray();
        int[] lcpArray = suffixTreeFromArray.computeLCPArray("GTAGT$", new int[]{5, 2, 3, 0, 4, 1});
